...
> 基于 **Vue 3 + FastAPI + MySQL** 的全栈观鸟平台，提供鸟类识别、社交分享、百科查询等功能。
...
| 密码 | **PBKDF2_SHA256** (passlib) | 无需编译 C 扩展，兼容性好 |
...
- **前端**：Node.js 18+
- **后端**：Python 3.10+，MySQL 8.0+（可选 — 无 MySQL 时自动降级为 SQLite）
...
| 表 | 说明 | 核心字段 |
|----|------|----------|
| `users` | 用户 | username, password_hash, nickname, avatar, bio |
| `posts` | 帖子 | user_id(FK), title, content, images(JSON), location |
| `post_likes` | 点赞记录 | user_id(FK), post_id(FK), UNIQUE(user_id, post_id) |
| `comments` | 评论 | user_id(FK), post_id(FK), content |
| `birds` | 鸟类百科 | name, latin_name, region, habits, description, image_url |
| `recognition_records` | 识别记录 | user_id(FK), image_url, result(JSON) |
