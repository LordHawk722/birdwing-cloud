...
async function handleRegister() {
  ...
  loading.value = true
  try {
    const res = await UserService.register(form.username.trim(), form.password)
    ...
    if (token && user) {
      ...
      serverError.value = '注册失败，服务器返回数据异常'
    }
  } catch (err) {
    const status = err?.statusCode
    if (status === 400) {
      serverError.value = err?.message || '用户名已存在'
    } else if (status === 422) {
      serverError.value = '输入格式不正确，请检查用户名和密码'
    } else {
