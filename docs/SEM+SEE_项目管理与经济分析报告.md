...
| 预计工期 | 15 周（2026-03-18 至 2026-06-30） |
| 关键路径长度 | 81 天（约 11.6 周） |
...
| 任务 | 起止日期 | 工期 | 负责人 |
|------|---------|:----:|------|
| 原项目结构分析 | 03-18 ~ 03-25 | 8天 | 技术负责人 + 开发团队 |
| 编写 POS | 03-23 ~ 03-29 | 7天 | 产品经理 |
| 编写 Project Charter | 03-27 ~ 04-02 | 7天 | 项目经理 |
| 成本预估 | 03-30 ~ 04-05 | 7天 | 项目经理 |
| 甘特图绘制 | 04-03 ~ 04-08 | 6天 | 项目经理 |
| 技术方案设计 | 04-01 ~ 04-09 | 9天 | 技术负责人 |
...
| 任务 | 起止日期 | 工期 | 负责人 |
|------|---------|:----:|------|
| 安全问题处理 | 05-21 ~ 06-03 | 14天 | 后端开发团队 |
| 测试环节 | 05-28 ~ 06-20 | 24天 | 测试工程师 + 开发团队 |

**项目交付阶段（2026-06-21 ~ 2026-06-30）**

| 任务 | 起止日期 | 工期 | 负责人 |
|------|---------|:----:|------|
| 项目交付 | 06-21 ~ 06-30 | 10天 | 项目经理 + 全体成员 |

#### 2.3.2 CPM 关键路径分析

![关键路径图](./imgs/关键路径图.jpg)

采用关键路径法（Critical Path Method）对 27 个任务节点进行正/逆向推算：

**关键路径（6 条并行关键路径，总长度 81 天 / 约 11.6 周）**：

```
路径 1（主关键路径）：
A → B → D → F → G → L → M → T → U → V → X → Y → Z → AA

路径 2：
A → B → D → F → G → H → J → M → T → U → V → X → Y → Z → AA

路径 3：
A → B → D → F → G → H → K → N → T → U → V → X → Y → Z → AA

路径 4：
A → B → D → F → G → L → P → T → U → V → X → Y → Z → AA

路径 5：
A → B → D → F → G → L → R → S → T → U → V → X → Y → Z → AA

路径 6：
... → T → U → W → X → Y → Z → AA
```

**关键任务节点**（23 个关键任务，占总任务 85%）：

| ID | 任务 | 工期 | ES | EF | LS | LF | TF |
|:--:|------|:---:|:--:|:--:|:--:|:--:|:--:|
| A | 需求分析 | 5 | 0 | 5 | 0 | 5 | **0** |
| B | 技术方案设计 | 5 | 5 | 10 | 5 | 10 | **0** |
| C | 项目计划制定 | 5 | 5 | 10 | 5 | 10 | **0** |
| D | 开发环境配置 | 5 | 10 | 15 | 10 | 15 | **0** |
| F | 后端接口适配 | 5 | 15 | 20 | 15 | 20 | **0** |
| G | 数据库设计优化 | 5 | 20 | 25 | 20 | 25 | **0** |
| H | 用户认证模块 | 10 | 25 | 35 | 25 | 35 | **0** |
| J | 鸟类识别模块 | 10 | 35 | 45 | 35 | 45 | **0** |
| K | 鸟类百科模块 | 10 | 35 | 45 | 35 | 45 | **0** |
| L | API 接口开发 | 20 | 25 | 45 | 25 | 45 | **0** |
| M | 社区分享模块 | 10 | 45 | 55 | 45 | 55 | **0** |
| N | AI 聊天模块 | 10 | 45 | 55 | 45 | 55 | **0** |
| P | 地图模块 | 10 | 45 | 55 | 45 | 55 | **0** |
| R | 文件存储服务 | 5 | 45 | 50 | 45 | 50 | **0** |
| S | 安全性增强 | 5 | 50 | 55 | 50 | 55 | **0** |
| T | 单元测试 | 5 | 55 | 60 | 55 | 60 | **0** |
| U | 集成测试 | 5 | 60 | 65 | 60 | 65 | **0** |
| V | 系统测试 | 5 | 65 | 70 | 65 | 70 | **0** |
| W | 用户验收测试 | 5 | 65 | 70 | 65 | 70 | **0** |
| X | 部署准备 | 3 | 70 | 73 | 70 | 73 | **0** |
| Y | 应用部署 | 3 | 73 | 76 | 73 | 76 | **0** |
| Z | 上线验证 | 3 | 76 | 79 | 76 | 79 | **0** |
| AA | 上线发布 | 2 | 79 | 81 | 79 | 81 | **0** |
...
|:--:|------|:--:|---------|
| E | 基础框架搭建 | 5天 | 可适当延后 |
| I | 首页模块 | 35天 | 优先级较低，灵活安排 |
| O | 排行榜模块 | 10天 | 任务间隙完成 |
| Q | 个人中心模块 | 20天 | 任务间隙完成 |

**关键路径管理重点**：
- **L - API 接口开发（20 天）**：工期最长，影响 5 个后续任务，需重点监控
- **H - 用户认证模块（10 天）**：基础模块，影响识别、百科、个人中心
- **测试阶段（T→U→V→W）**：连续关键任务，无缓冲时间
- 关键任务延迟需立即启动应急预案
...
2. **计划完整**：基于 WBS（7 阶段、27 个二级任务、120+ 个三级任务）与甘特图（2026-03-18 至 2026-06-30，15 周），CPM 关键路径长度 81 天，23 个关键任务，6 条并行关键路径。团队配置 9~11 人，4 人并行开发。
